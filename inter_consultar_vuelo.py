...
qtCreatorFile = "inter_consultar_vuelo.ui" # Nombre del archivo que creamos en Qt dessigener en nuestro caso calculadora_suma.iu.
Ui_MainWindow, QtBaseClass = uic.loadUiType(qtCreatorFile)# nose que hace pero siempre va jajajaja
class Consulta_vuelo(QtWidgets.QMainWindow, Ui_MainWindow):# nobre de la clase como tal seiempre igual
    ...
    def consultar(self):
        ...
        if b1:
            if self.verifica_vuelo(id_vuelo):
                ...
                self.cursor.execute("select * from vuelos where id_vuelo ='"+id_vuelo+"';")  
